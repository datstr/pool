...
export class Pool {
  ...
  async retargetAssignments() {
    ...
    for (const master of connected) {
      ...
      d = Math.max(p.minDifficulty, Math.min(maxD, this.networkDifficulty() || Infinity, Number(d.toPrecision(3)))); // never above what a block takes, never below the pool's floor
